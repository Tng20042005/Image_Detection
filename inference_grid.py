...
# Danh sách các ngưỡng chạy độc lập tuần tự
...
def load_model(device):
    ...
    if not os.path.exists("./models/best_loss3.pth"):
        print("⚠️ Cảnh báo: Không tìm thấy './models/best_loss3.pth'!")
    else:
        state = torch.load("./models/best_loss3.pth", map_location="cpu")
...
def main(args):
    ...
    print("🚀 Grid Test Device:", device)

    torch.backends.cudnn.benchmark = True 
    model = load_model(device)
    dataset = ImageDataset(args.image_dir)

    # Xử lý phần mở rộng của đường dẫn đầu ra
    base_output_path, ext = os.path.splitext(args.output)
    if not ext:
        ext = ".json"

    # VÒNG LẶP LỚN NHẤT: Chạy tuần tự và độc lập từng ngưỡng Threshold
    for thresh in THRESHOLDS:
        print("\n" + "="*50)
        print(f"🔥 STARTING PIPELINE WITH THRESHOLD: {thresh:.2f}")
        print("="*50)

        # Khởi tạo DataLoader mới cho lượt chạy này
        loader = DataLoader(
            dataset,
            batch_size=32, 
            shuffle=False,
        ...
        with torch.no_grad():
            for imgs, names, ws, hs, scales, pls, pts in loader:
                ...
                for i in range(B):
                    ...
                    # Khâu Decode nhận giá trị thresh sạch của lượt chạy hiện tại
                    ...
                    # Khâu NMS nhận giá trị thresh sạch của lượt chạy hiện tại
                    nms_boxes = nms(raw_boxes, 0.5, thresh)
                    ...
                    current_thresh_results.append(img_result)

                # Progress Bar cho lượt chạy hiện tại
                processed += len(names)
                ...
                print(
                    f"⏳ [Thresh {thresh:.2f}] Processing: {processed}/{total} "
        ...
        # Lưu NGAY LẬP TỨC kết quả của ngưỡng vừa chạy xong ra file JSON riêng biệt
        specific_output_path = f"{base_output_path}_thresh_{thresh:.2f}{ext}"
        with open(specific_output_path, "w", encoding="utf-8") as f:
            json.dump(current_thresh_results, f, indent=2)
            
        print(f"\n✅ Finished & Saved: {specific_output_path}")

    print("\n🎉 All 11 independent threshold runs have completed successfully!")

# ================= RUN =================
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--image_dir", type=str, required=True, help="Đường dẫn thư mục ảnh")
    parser.add_argument("--output", type=str, required=True, help="Tên file JSON gốc (Ví dụ: result.json)")
